...
- 每个环境从 row 0 开始，满足接近完整回合、无失败和净位移条件才积累成功次数。
- 连续两次成功才升阶；失败才降阶。
...
![盲走楼梯全地形演示](docs/media/stairs-blind-terrain-demo.gif)

![盲走训练地形](docs/media/stairs-terrain.png)

### 附加视觉楼梯演示

以下 GIF 是视觉上楼梯/视觉识别楼梯的补充演示，不属于当前 DWAQ 盲行走 actor 的输入契约；当前 DWAQ actor 仍只使用本体感觉、命令、历史和步态相位。

![视觉上楼梯演示](docs/media/stairs-vision-up-demo.gif)

![视觉识别楼梯演示](docs/media/stairs-vision-recognition-demo.gif)

原始 MP4 保留在 `docs/media/`，用于下载和复核。
